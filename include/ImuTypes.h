...
// imu在k时刻的测量->加速度,角速度,时间戳
class Point
{
public:
    Point(const float &acc_x, 
          const float &acc_y, 
          const float &acc_z,     // 加速度
          const float &ang_vel_x, 
          const float &ang_vel_y, 
          const float &ang_vel_z, // 角速度
...
class Bias
{
    // ?序列化->将对象变成字节流的形式->可用于保存到本地文件或网络传输
    // 入侵式的序列化
    friend class boost::serialization::access;
    template<class Archive>
    // 为了序列化用户自定义的对象，必须定义成员函数serialize()
    /**
     * @brief 在恢复和转存的时候都会调用
     * 
     * @param ar       Boost 归档文件的引用
     * @param version 
     */
