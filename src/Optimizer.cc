                                 ...
                                 const bool bRobust)
{
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(leftIndex != -1 &&      // 左目中有特征点
            pKF->mvuRight[get<0>(mit->second)]<0) // 右目中的索引为-1
            // 这个时候证明是在左目中
            {
            ...
            if(pKF->mpCamera2){
                ...
                // 索引存在
                if(rightIndex != -1 && 
                   rightIndex < pKF->mvKeysRight.size()){ // 且小于右目的特征数量
                   // 右目的索引????????????
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        // LoopKF==0
        // 闭环帧不可能是初始帧,这个时候是在初始化的时候调用的全局优化的函数
        // 这个时候只有两个关键帧
