 ...
 * @return                   成功匹配的数量
 */
// 在ORB的代码结构中,如果是指针那么这个变量会被长久的存储下来,如果是引用一般表示一个临时变量.
...
/**
 * @brief 得到更新后的匹配关系  但是这里只要了直方图中点数最多的三个bin的点,其他的都不要了.
 * 
 * @param F1 
 * @param F2 
 * @param vbPrevMatched 
 ...
 * @return int 
 */
int ORBmatcher::SearchForInitialization(Frame &F1, // 初始帧
                                        Frame &F2, // 当前帧
                                        vector<cv::Point2f> &vbPrevMatched, // 初始帧中的特征点 
                                        vector<int> &vnMatches12,  // 现在全是-1
                                        int windowSize) // 100
{
    int nmatches=0;
    // 1-> 2的匹配
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        // 利用F2的特征点找到在F1中该特征点周围的特征点并返回了索引
        vector<size_t> vIndices2 = F2.GetFeaturesInArea(vbPrevMatched[i1].x,  // 初始帧去畸变的特征点x
                                                        vbPrevMatched[i1].y,  // 初始帧去畸变的特征点y
                                                        windowSize, // 100
                                                        level1,     // 特征点的层数,这里只能是0
                                                        level1);
...
/**
 * @brief 计算直方图中值最大的三个index,如果最大的远大于第二个则只保留最大的.
 * 
 * @param histo 旋转直方图
 * @param L     直方图划分区间,30度,12度一个bin
 * @param ind1  最多点的bin的索引
 * @param ind2 
 * @param ind3 
 */
...
// ? https://zhuanlan.zhihu.com/p/32899882
